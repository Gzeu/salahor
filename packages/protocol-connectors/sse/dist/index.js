...
export function createSseClient(url, options = {}) {
    ...
    const { autoReconnect = DEFAULT_OPTIONS.autoReconnect, reconnectDelay = DEFAULT_OPTIONS.reconnectDelay, maxReconnectAttempts = DEFAULT_OPTIONS.maxReconnectAttempts, withCredentials = DEFAULT_OPTIONS.withCredentials, } = options;
            ...
            source.addEventListener = function (type, listener) {
                ...
                const wrappedListener = (event) => {
                    if (type === 'message' && 'handleEvent' in listener) {
                        listener.handleEvent(new MessageEvent('message', {
                            data: event.data,
                ...
                eventSource[`on${type}`] = wrappedListener;
            };
        }
...
export function createSseServer(options = {}) {
    const { headers = { 'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive' }, keepAlive = true, keepAliveInterval = 30000, maxConnections = Infinity, } = options;
    const connections = new Map();
    const connectionsStream = createEventStream();
