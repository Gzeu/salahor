 ...
 */
export async function fileExists(filePath, options = {}) {
    const { cwd = process.cwd(), checkReadable = true, checkWritable = false, checkExecutable = false, } = options;
...
export function fileExistsSync(filePath, options = {}) {
    const { cwd = process.cwd(), checkReadable = true, checkWritable = false, checkExecutable = false, } = options;
    const resolvedPath = filePath.startsWith('/')
        ? filePath
        : join(cwd, filePath);
